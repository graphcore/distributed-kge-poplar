...
def _random_batch(engine: kge.Engine) -> kge.Batch:
    ...
    return kge.Batch(
        ...
        weight=(
            engine.random.uniform(size=bshape + (s.data.batch_size, s.n_tail))
        ).astype(s.execution.dtype),
    )
...
def test_engine_ops() -> None:
    ...
    loss_0 = engine.train_step_loop(random_batch)
    loss_1 = engine.train_step_loop(random_batch)
...
def test_training(dtype: str) -> None:
    ...
    # MRR check
    np.testing.assert_allclose(
        mrr, reference_mrr, rtol=dict(float16=2e-3, float32=1e-4)[dtype]
    )
