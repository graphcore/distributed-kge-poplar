...
struct Model {
    ...
    EntityEmbedding readEntityData(const fr::Tensor& indices) {
    ...
    void writeEntityData(const EntityEmbedding& entities, const fr::Tensor& indices) {
    ...
    fr::Tensor getLoss(const fr::Tensor& scores,
                       const fr::Tensor& tailIndices,
                       fr::Tensor& weight) {
        if (negativeAdversarialScale > 0) {
            fr::Frame f;
            auto oneHotTailMask = fr::ops::oneHot(tailIndices, nShard * a2aSize, poplar::FLOAT);
            auto negScoreSoftmax =
                detachedSoftmax((fr::ops::constant(negativeAdversarialScale, poplar::FLOAT) *
                                 fr::Tensor::wrap(f.graph.wrap(f.graph.unwrap(scores.pag()),
                                                               /*requiresGrad*/ false))) +
                                (fr::ops::constant(-10000.0f, poplar::FLOAT) * oneHotTailMask));
            weight =
                weight * (oneHotTailMask +
                          fr::ops::constant(nShard * a2aSize - 1, poplar::FLOAT) * negScoreSoftmax);
        }

        if (lossFn == "logsigmoid") {
            auto tailMask = fr::ops::constant(2.0f, poplar::FLOAT) *
                                fr::ops::oneHot(tailIndices, nShard * a2aSize, poplar::FLOAT) -
                            fr::ops::constant(1.0f, poplar::FLOAT);
            ...
            // This loss doesn't support elementwise weighting
            return fr::ops::sum(fr::ops::sum(weight, {1}) *
                                fr::nn::softmaxCrossEntropy(correctedScores, tailIndices));
    ...
    fr::Tensor trainStep(const fr::Tensor& learningRate,
        ...
        auto entities = readEntityData(remoteIndices);
        ...
        // 4. Compute loss
        auto loss = getLoss(scores, tailIndices, weight) + regularisationLoss;
        ...
        writeEntityData(entities, remoteIndices);
    ...
    void trainStepLoop() {
        ...
        auto weight = fr::ops::input("weight", {{nStep, batchSize, nShard * a2aSize}, dtype});
        ...
        fr::ops::forN(nStep, [&](const fr::Tensor& index) {
            fr::Frame f;
            auto weightSlice = switchedSlice(weight, index).astype(poplar::FLOAT) /
                               fr::ops::constant<float>(nShard * batchSize * a2aSize * nShard);
            auto stepLoss = trainStep(
                learningRate, switchedSlice(remoteIndices, index), switchedSlice(a2aIndices, index),
                switchedSlice(headIndices, index), switchedSlice(relationIndices, index),
                switchedSlice(tailIndices, index), weightSlice);
    ...
    void predict() {
        ...
        auto predictedTails =
            fr::ops::allGather(predictTail(entityHiddenPredict(readEntityData(headIndices), "head"),
                                           relationIndices))
        ...
        fr::ops::forN(nLoop, [&](const fr::Tensor& index) {
            ...
            auto tails = entityHiddenPredict(
                readEntityData(tailIndices * tailMask.astype(poplar::UNSIGNED_INT)), "tail");
            auto scores =
    ...
    void writeEntity() {
        ...
        auto data = fr::ops::input(
            "write_data", {{rwBatchSize, 3 * entityEmbeddingSize + entityFeatureSize}, dtype});
        entityData.write(data, indices);
    }
};
