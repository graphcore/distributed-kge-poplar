...
class Dataset:
    def __init__(
        ...
        # Precomute 'tail' and 'weight', possible since batch positives are always in the same place
        tail = (
        ...
        self.n_tails = n_shard * settings.a2a_size
        if isinstance(self.settings.sample_weight, kge.HrtFrequencyWeight):
            # Weight positive triples based on (h,r) and (r,t) frequency in dataset
            _, hr_idx, hr_count = np.unique(
                train_hrt_sorted[:, 0] + self.data.n_entity * train_hrt_sorted[:, 1],
                return_counts=True,
                return_inverse=True,
            )
            _, rt_idx, rt_count = np.unique(
                train_hrt_sorted[:, 2] + self.data.n_entity * train_hrt_sorted[:, 1],
                return_counts=True,
                return_inverse=True,
            )
            self.triple_weights = np.sqrt(
                1.0
                / (
                    hr_count[hr_idx]
                    + rt_count[rt_idx]
                    + self.settings.sample_weight.smoothing
                )
            ).astype(self.weight_dtype)
            self.tail_mask = self.pos_tail_idx[..., np.newaxis] == np.arange(
                self.n_tails
            )
        else:
            # Positive & negative have the same total contribution (same for all triples)
            positive_weight = 0.5 * self.n_tails
            negative_weight = 0.5 * self.n_tails / (self.n_tails - 1)
            self.tail_mask = tail[..., np.newaxis] == np.arange(self.n_tails)
            weight = np.where(
                self.tail_mask,
                positive_weight,
                negative_weight,
            ...
            self.weight = np.tile(
                weight[:, np.newaxis, :, :], (1, train_steps_per_program_run, 1, 1)
            )
        # Note that weight has been scaled inline by the overall number of tails
        # Therefore, where you would have originally summed across all tails,
        # you should take the mean in order to reverse this scaling.
    ...
    def load(cls, data: RawData, settings: kge.Settings) -> "Dataset":
        return cls(
            ...
            weight_dtype=settings.execution.dtype,
    ...
    def sample_batch(self) -> kge.Batch:
        ...
        if isinstance(self.settings.sample_weight, kge.HrtFrequencyWeight):
            positive_weight = self.triple_weights[sample_idx].reshape(
                n_shard, n_batch, batch_size, 1
            )
            positive_weight /= 2 * np.sum(positive_weight, axis=(0, 2), keepdims=True)
            positive_weight *= self.n_tails * n_shard * self.settings.batch_size
            negative_weight = (positive_weight / (self.n_tails - 1)).astype(
                self.weight_dtype
            )
            weight = np.where(
                self.tail_mask,
