...
@dataclasses.dataclass
class HrtFrequencyWeight:
    smoothing: float
    type: str = "hrt_frequency"
...
class DataSettings:
    ...
    sample_weight: Optional[HrtFrequencyWeight]
...
class Settings:
    ...
    @property
    def entity_data_size(self) -> int:
        return 3 * self.model.embedding_size + self.model.entity_feature_size

    @property
    def entity_data_rw_shape(self) -> Shape:
        return (self.model.n_shard, self.execution.rw_batch_size, self.entity_data_size)
    ...
    def create_demo(cls) -> "Settings":
        return cls(
            ...
            data=DataSettings(
                ...
                sample_weight=HrtFrequencyWeight(smoothing=0.0),
    ...
    def create_wikikg90mv2(cls) -> "Settings":
        ...
        return cls(
            ...
            training=TrainingSettings(
                ...
                learning_rate_modifiers=dict(entity_embedding=10),
                adam_beta_m=0.9,
                adam_beta_v=0.999,
                adam_epsilon=1e-8,
                weight_decay=0.0,
...
class Batch:
    ...
    weight: np.ndarray  # . float16|float32[n_shard x steps x batch_size x (n_shard * a2a_size)]
...
class Engine:
    ...
    def write_entity(self, offset: int, data: np.ndarray) -> None:
        ...
        self._check_shape(
            "write_entity(data)", data, self.settings.entity_data_rw_shape
        )
        indices = self._rw_indices(offset)
        self.engine.run(
            "write_entity",
            dict(write_indices=indices, write_data=self._int16_if_float16(data)),
        )
    ...
    def read_entity(self, offset: int) -> np.ndarray:
        ...
        value = np.zeros(self.settings.entity_data_rw_shape, dtype=dtype)
        self.engine.run(
            "read_entity",
    ...
    def initialise_all(self, entity_features: np.ndarray) -> None:
        ...
        for offset in range(0, self.settings.model.n_entity, rw_batch_size):
            ...
            embeddings = self.init_scale * self.random_normal(
                (
            ...
            self.write_entity(
                offset,
                np.concatenate(
                    [  # Construct [embedding, adam_m, adam_v, feature]
                        embeddings,
                        np.zeros_like(embeddings),
                        np.zeros_like(embeddings),
                        features_chunk.astype(dtype),
                    ],
                    2,
                ),
            )
